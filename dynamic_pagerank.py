...
def parse_args():
    ...
    parser.add_argument(
        '--explorative-method',
        nargs='?',
        default='dynamic_pagerank',
        help='Dynamic PageRank.')
...
def load_daily_data(sample_dir, day_id):
    ...
    daily_nx = sample_dir + 'day_nx_' + str(day_id)
    return users, articles, labels, creators, daily_nx
...
def sean_training(embedding_matrix):
    ...
    nx_G = read_graph('karate.edgelist')
    ...
    val_results = {"acc": [], "auc": [], "f1": [], "precision": [], "recall": []}
    test_results = {"acc": [], "auc": [], "f1": [], "precision": [], "recall": []}
    ...
    for day in range(args.day_count - 1):
        ...
        train_key = day + 1
        train_val_users, train_val_articles, train_val_labels, train_val_creators, train_val_nx = load_daily_data(
        ...
        print("training...")
        if train_key > 1:
            nx_G = read_graph(train_val_nx)
            node_pr = nx.pagerank(nx_G, alpha=0.9)
            for node in node_pr:
                ucb_G.acc_history[node] = [node_pr[node]]
        ...
        print("testing...")
        test_key = day + 2
        test_users, test_articles, test_labels, test_creators, test_nx = load_daily_data(
