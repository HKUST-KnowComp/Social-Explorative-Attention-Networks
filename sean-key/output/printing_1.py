...
def main():
	...
	for i in range(3, day_count):
		day_id = str(i)
		users, labels, creators = load_daily_data('../processed_feed/', day_id)
		...
		for j in range(len(d["prediction"][day_id])):
			if d["prediction"][day_id][j][0] > 0.4:
	...
	gini_x = round(float(np.mean(pred_creator_gini_index)), 4)
	f1_y = round(d["avg_test"]["f1"], 4)
	h_m = harmonic_mean(gini_x, f1_y)
	print("predict_gini:", np.mean(pred_creator_gini_index))
	print("user count:", len(dict(Counter(user_count))))
	print("f1", f1_y, "gini_avg", gini_x, "C&C:", h_m)
	"""
	...
	"""
	# for i in range(1, len(d["running_time"])):
	# 	print(i + 2, np.log10(d["running_time"][str(i)]))
	print(d["param"])
