...
def parse_args():
    ...
    parser.add_argument('--batch-size', type=int, default=64,
                        help='Minibatch for training. Default is 64.')
...
def build_social_sample(walks, users, articles, labels):
    ...
    d_user = pad_sequences(d_vectorized_sample[:, 0], maxlen=args.walk_length, padding='post')
...
def random_walks(x):
    ...
    for walk_iter in range(args.num_walks):
        for node in nodes:
                walk = [node]
		walk = list(
                walk.extend(np.random.randint(
                    1,
                    args.max_user,
                    size=args.walk_length))
...
def sean_training(embedding_matrix):
    ...
    node_pr = nx.pagerank(ucb_G.G, alpha=0.9)
    for node in init_acc_history:
        init_acc_history[node].append(node_pr[node])
